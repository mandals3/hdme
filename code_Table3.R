...
for(i in 1:nSimu){
	...
	for(j in 1:nNCausalMarker){
		...
		for(k in 1:nLambda){
			...
			thetaVec1 <- rbeta(n.marker, 3, 3) #theta ~ Beta(3, 3)
			thetaVec2 <- rbeta(n.marker, 5, 2) #theta ~ Beta(5, 2)
			X1 <- X + rnorm(n.sample * n.marker, 0, rep(thetaVec1, each = n.sample))
			X2 <- X + rnorm(n.sample * n.marker, 0, rep(thetaVec2, each = n.sample))
			s <- (j - 1) * nLambda + k
...
#write.table(tempWrite, file = "result_Table3.txt", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
